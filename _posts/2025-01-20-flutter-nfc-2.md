...
> [ISO/IEC 14443이란?](https://namu.wiki/w/ISO/IEC%2014443){: target="\_blank"}  
> 식별에 사용되는 프록시미티 카드, 해당 카드와 통신하기 위한 전송 프로토콜을 정의하는 국제표준이다.
...
> <a href='https://www.ssl.com/ko/%EC%9E%90%EC%A3%BC-%EB%AC%BB%EB%8A%94-%EC%A7%88%EB%AC%B8/%ED%83%80%EC%9B%90-%EA%B3%A1%EC%84%A0-%EC%95%94%ED%98%B8%ED%99%94-ECC-%EB%9E%80/' target='_blank'>ECC 란?</a>  
> 타원 곡선 암호화 (ECC) 타원 곡선의 수학적 속성을 사용해 공개키 암호화시스템을 생성하는 방식  
> ECC는 한방향으로 계산은 쉽지만, 되돌리기는 어렵다.  
> ECC 의 경우, 랜덤 타원 곡선 요소의 이산 로그, 타원곡선이산로그 문제를 계산할수 없다는 어려움이 있다.  
> 그만큼 타원 곡선 디지털 서명 알고리즘을 사용하는 공개키 암호화가 널리 사용되는 서명 알고리즘이다.
